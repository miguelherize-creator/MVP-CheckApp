...
export interface CartolaSantanderRow {
  ...
  debit: number | null;   // cargo
  credit: number | null;  // abono
  description: string;
  balance: number | null;
  docNumber: string;
  branch: string;
...
function classifyMovement(credit: number | null, debit: number | null, desc: string): 'income' | 'expense' {
  if (credit !== null && debit === null) return 'income';
  if (debit !== null && credit === null) return 'expense';
  const d = desc.toLowerCase();
  return d.includes('abono') || d.includes('depósito') || d.includes('reverso') ? 'income' : 'expense';
}
...
function parseHeader(lines: PdfLine[]): CartolaSantanderHeader {
  const header: CartolaSantanderHeader = {
    ...
    currency: 'CLP', dateFrom: null, dateTo: null,
...
function parseRows(lines: PdfLine[]): CartolaSantanderRow[] {
  ...
  for (const line of lines) {
    ...
    if (strs.includes('FECHA') && strs.includes('CARGO') && strs.includes('ABONO')) {
      colX = {};
      for (const item of line.items) colX[item.str] = item.x;
      break;
    }
  }
  ...
  const flush = () => {
    ...
    rows.push({
      date: cur.date, debit: cur.debit, credit: cur.credit,
      description: desc, balance: cur.balance,
      docNumber: cur.docNumber, branch: cur.branch,
  ...
  for (const line of lines) {
    ...
    if (DATE_RE.test(first.str)) {
      ...
      for (const item of line.items.slice(1)) {
        const { x, str } = item;
        if (Math.abs(x - cargoX) <= TOL && AMOUNT_RE.test(str))       cur.debit = parseChileanAmount(str);
        else if (Math.abs(x - abonoX) <= TOL && AMOUNT_RE.test(str))  cur.credit = parseChileanAmount(str);
        else if (Math.abs(x - saldoX) <= TOL && AMOUNT_RE.test(str))  cur.balance = parseChileanAmount(str);
        else if (x >= sucX - TOL)                                       cur.branch = str;
        else if (x >= docX - TOL)                                       cur.docNumber = str;
        else if (x >= descX - TOL)                                      cur.descParts.push(str);
      }
    } else if (cur) {
      for (const item of line.items) {
        if (item.x >= descX - TOL && item.x < saldoX) cur.descParts.push(item.str);
      }
    }
  }
  flush();
...
export async function parseSantanderCartola(pdfBuffer: Buffer): Promise<CartolaSantanderResult> {
